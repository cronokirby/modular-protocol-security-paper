...
Given two systems, $A$ and $B$, we can define their tensor product $A * B$,
which is any system satisfying:
$$
\tx{SysPack}(A * B) = (\tx{NoChan}(A) \otimes \tx{NoChan}(B)) \circ
\begin{pmatrix}
...
$A \circ B$ as any system satisfying:
\todo{add constraints like for packages}
$$
...
Analogously to games, we define a \emph{system game} as a pair of systems $S_b$
with $\tx{In}(S_b) = \emptyset$.
\end{definition}

\begin{definition}[System Game Reductions]
We can also define notions of reductions for system games.

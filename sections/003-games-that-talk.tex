...
\begin{definition}[Asynchronous Package Equality]
...
\begin{lemma}[Equality is Transitive]
  ...
  \txbf{Proof:} A key component of this proof is that delayers compose.
  ...
  Then, by unrolling $\varepsilon$, and using a union bound \todo{reference},
  we get:
  $$
...
\begin{lemma}[Tensoring Respects Equality]
  Given asynchronous systems $A, B, B'$, we have:
  \begin{enumerate}
    \item $B = B' \implies A \otimes B = A \otimes B'$,
    \item $B \overset{\epsilon}{=} B' \implies A \otimes B \overset{\epsilon}{=} A \otimes B'$,
  \end{enumerate}
  provided that these expressions are well defined.
...
\begin{lemma}[Horizontal Composition Respects Equality]
\end{lemma}

...
The protocol using this functionality is then called $\mathcal{Q}$,
...
The full protocol we consider is $\mathcal{Q} \lhd (\mathcal{P}[\tx{IdealRand}] \circ F[\tx{Add}])$,
which can perfectly simulate $\mathcal{P}[\tx{Rand}]$, as we now prove.

\begin{claim}
    Let $\mathcal{C}$ be the class of corruptions where up to $n - 1$ parties
    are corrupt.
    Then it holds that:
    $$
    \mathcal{P}[\tx{Rand}] \overset{0}{\leadsto}_{\mathcal{C}} \mathcal{Q} \lhd (\mathcal{P}[\tx{IdealRand}] \circ F[\tx{Add}])
    $$
    ...
    We start by unrolling $\tx{Inst}_C(\mathcal{P}[\tx{Rand}])$, to get:
    ...
    semi-honest party of $\mathcal{Q}$.
    ...
    $$
    \tx{Inst}_C(\mathcal{P}[\tx{Rand}]) = \tx{SimInst}_{S, C}(\mathcal{Q} \lhd (\mathcal{P}[\tx{IdealRand}] \circ F[\tx{Add}]))
    $$

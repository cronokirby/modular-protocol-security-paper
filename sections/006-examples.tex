...
\package{$F[\tx{PrivChan}]$}{
    &m_{1\to2}, m_{2\to1} \gets \tx{FifoQueue}.\tx{new}()\cr
...
Rather than being able to see the current state of either message queue,
...
\inlinepackage{$\Gamma^5$}{
    &l_{1\to2}, l_{2\to1} \gets \tx{FifoQueue}.\tx{new}()\cr
    &\txbf{view } c_{1\to2}, c_{2\to1} \gets \tx{FifoQueue}.\tx{new}()\cr
    &m_{1\to2}, m_{2\to1} \gets \tx{FifoQueue}.\tx{new}()\cr
    &(\tx{sk}_i, \tx{pk}_i) \xleftarrow{\$} \tx{Gen}()\cr
    \cr
...
\Gamma^2 =
\inlinepackage{$\Gamma^3$}{
    &m_{1\to 2}, \txbf{view } c_{1\to2}, \txbf{view } c_{2\to1} \gets \tx{FifoQueue}.\tx{new}()\cr
    &(\tx{sk}_1, \tx{pk}_1),\
    (\tx{sk}_2, \tx{pk}_2),\
    (\tx{sk}'_2, \tx{pk}'_2) \gets \tx{Gen}()\cr
    \cr
...
and then create a simulated ciphertext by encrypting it.

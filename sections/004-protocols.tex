...
Given a system $P$, and an agent $A$ for $P$, we can define
the semi-honest corruption $\tx{Corrupt}_{\tx{SH}}^A(P)$.

First, we need to define $\tx{Logged}(P)$, which is a transformation
...
Finally, we define:
$$
\tx{Corrupt}_{\tx{SH}}^A(P) := (A \otimes 1(\{\tx{log}\})) \circ \tx{Logged}(P)
$$
...
is an association between each player $P_i$ and a \emph{corruption} $C_i$,
which is one of:
\begin{itemize}
  \item Honest, with an agent $A$ for $P_i$,
  \item Semi-Honest, with an agent $A$ for $P_i$,
  \item Malicious.
\end{itemize}
Given a corruption model $C$ we can define $\tx{Corrupt}_{C}(P_i)$
as one of:
\begin{itemize}
  \item $\tx{Corrupt}_H^A(P)$
  \item $\tx{Corrupt}_{\tx{SH}}^A(P)$
  \item $\tx{Corrupt}_{M}(P)$
\end{itemize}
depending on the type of corruption from the association.

Corruption models have a natural partial order associated with them. 
We have:
$$
\tx{Honest} < \tx{SemiHonest} < \tx{Malicious}
$$
and then we say that an association $C_1, \ldots, C_n$ is $\geq$
than another $C'_1, \ldots, C'_n$ if $\forall i.\ C_i \geq C'_i$.

A \emph{class of corruptions} $\mathcal{C}$ is simply a set of corruption models.
We say that such a class is \emph{closed} if whenever it contains
an honest (resp. semi-honest) corruption for a player $P_i$ with agent $A$,
then it also contains honest (resp. semi-honest) corruptions for $P_i$
with every efficient agent $A'$.
Unless otherwise specified, we only consider \emph{closed} classes.
For closed classes, we defined a partial order, via:
$$
\mathcal{C} \geq \mathcal{C'} \iff \forall C' \in \mathcal{C'}. \exists C \in \mathcal{C}.\ C \geq C'
$$
...
\begin{definition}[Instantiation]
  Given a protocol $\mathcal{P}$ and a corruption model $C$, we can
...
is the largest (closed) corruption class $\mathcal{C}'$ such

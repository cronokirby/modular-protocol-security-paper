...
\begin{theorem}[Horizontal Composition Theorem]
  ...
  In fact, the only way $S$ is used by $\mathcal{P}.P_i$ is via
  the maliciously corrupt players, which can call functions exported
  by the $\mathcal{Q}'.P_i$ via $S$.
  However, because $1(O)$ does \emph{not} provide access to
  $\tx{Call}_F$ for each of these maliciously corrupt $\mathcal{P}.P_i$,
  we can effectively replace them with equivalent systems that don't
  have these functions, and thus do not depend on $S$.
  In conclusion, we can then write:
  $$
  \footnotesize
  1(O) \circ
  \begin{pmatrix}

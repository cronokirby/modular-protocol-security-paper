...
First, in order to allow access to the randomness used by $P$,
we define a transformation $\tx{NoRand}(P)$ which replaces
each internal bit flip with an external call $\tx{Flip}()$.
We then define the following game:
\begin{game}{game:sharedrand}{SharedRand}
\package{SharedRand}{
&b[1, \ldots] \gets \bot\cr
&i, j \gets 0\cr
\cr
&\underline{\tx{Flip}()}\cr
\pind{1} i \gets i + 1\cr
\pind{1} \txbf{if } b[i] = \bot \tx{: } b[i] \xleftarrow{\$} \{0, 1\}\cr
\pind{1} \txbf{return } b[i]
\cr
&\underline{\tx{Next}()}\cr
\pind{1} j \gets j + 1\cr
\pind{1} \txbf{if } b[j] = \bot \tx{: } b[j] \xleftarrow{\$} \{0, 1\}\cr
\pind{1} \txbf{return } b[j]
}
\end{game}

One interesting thing with this game is that it's not possible
to observe any information about the number of calls made to $\tx{Flip}$
just by calling $\tx{Next}$, since the latter does not depend on $i$.

We also define a transformation $\tx{Logged}$, which instruments
$P$ to log the use of channels and external functions.
More formally, $\tx{Logged}(P)$ is a system which works the same
...
Additionally, $\tx{Logged}(P)$ modifies $P$ by pushing events to these
...
First, note that if $P = P'$ as systems, then $\tx{Logged}(P) = \tx{Logged}(P')$,
because $\tx{NoChan}(P) = \tx{NoChan}(P')$.
One aspect in which they may differ is their use of randomness.
Indeed, it's possible that the two systems are equal despite flipping bits
in a different way. Fortunately, the calls to $\tx{Flip}$ do not
affect the calls to $\tx{Next}$, so these differences are not observable.
...
\begin{definition}[Instantiation]
  Given a protocol $\mathcal{P}$ with $\text{In}(\mathcal{P}) = \emptyset, \text{IdealIn}(\mathcal{P}) = \emptyset$ and a corruption model $C$, we can
...
\begin{theorem}[Horizontal Breakdown]
  ...
  \begin{pmatrix}
    \mathcal{P}.F\cr
    \otimes\cr
    \bigotimes_{i \in [\mathcal{Q}.n]} S_i
  \end{pmatrix}
  \circ
  \tx{Inst}_C(\mathcal{Q})
  $$
...
\begin{pmatrix}
  \tx{Corrupt}_M(P)\cr
  \otimes\cr
  1(\tx{In}(\tx{Corrupt}_M(Q_1)), \ldots)\cr
  \otimes\cr
  1(\tx{In}(Q_1), \ldots)
\end{pmatrix}
\circ
\begin{pmatrix}
  ...
  *\cr
  \tx{Corrupt}_M(Q_m)\cr
  \otimes\cr
  1(\tx{In}(Q_1), \ldots, \tx{In}(Q_m))
\end{pmatrix}
$$
by definition, since corruption $P \circ (Q_1 * \cdots)$ precisely allows
...
using $1(L^M)$ for $L^M = \tx{In}(\tx{Corrupt}_M(Q_1)) \cup \cdots \cup \tx{In}(Q_1) \cup \cdots$.

Next, we look at semi-honest corruption.

First, we look at $\tx{Logged}$.
Because the logs are divided into independent sub logs, we can write:
$$
\tx{Logged}(P \circ (Q_1 * \cdots * Q_m)) =
1(O) \circ
\begin{pmatrix}
  \tx{Logged}(P)\cr
  \otimes\cr
...
\end{pmatrix}
$$
where $O = \tx{Out}(\tx{Logged}(P \circ (Q_1 * \cdots)))$

Second, note that
$$
\tx{NoRand}(P \circ (Q_1 * \cdots)) =
\tx{NoRand}(P) \circ
\begin{pmatrix}
  \tx{NoRand}(Q_1)\cr
  *\cr
...
Combining these two observations with the definition of $\tx{Corrupt}_{\tx{SH}}$,
we get:
$$
\begin{aligned}
  &\tx{Corrupt}_{\tx{SH}}(P \circ (Q_1 * \cdots)) =\cr
  &1(O) \circ
  \begin{pmatrix}
    \tx{NoRand}(\tx{Logged}(P \circ (Q_1 * \cdots)))\cr
    \otimes \cr
    1(\{\tx{Next}\})
  \end{pmatrix}
  \circ
...
\end{pmatrix}
  ...
  \begin{pmatrix}
    \tx{NoRand}(\tx{Logged}(P))\cr
    \otimes \cr
    1(\{Q_1.\tx{log}, \ldots\})\cr
    \otimes \cr
    1(\{\tx{Next}\})
  \end{pmatrix}
  \circ
  \begin{pmatrix}
    \tx{NoRand}(\tx{Logged}(Q_1))\cr
    * \cr
    \cdots\cr
    \otimes \cr
    1(\{\tx{Flip}, \tx{Next}\})
...
\end{aligned}
$$
for $O = \tx{Out}(\tx{Corrupt}_{\tx{SH}}(P \circ (Q_1 * \cdots)))$
(note that Lemma~\ref{thm:interchange_system} (interchange)
was implicitly applied several times in this series of equations).
